class User < ApplicationRecord
    ...
    def favorite_coffees
        ...
        self.ratings.joins(:user).where(rating: [4,5]).collect {|rating| rating.coffee}
    end

    def self.compare_favorite_coffees(current_user) # returns users with similar taste in coffee to the current user
        self.select {
            |u| current_user.favorite_coffees.any? {
